...
func TestGetNewAccessToken(t *testing.T) {
	...
	assert.False(t, at.IsExpired(), "brand new access token should not ne expired")
	assert.EqualValues(t, "", at.AccessToken, "new access token should not have defined access token id")
	assert.True(t, at.UserId == 0, "new access token should not have have an associated user id")
}
